...
X-WA-BIZ-DESCRIPTION:${((await this.getBusinessProfile(njid)).profile.description || '').replace(/\n/g, '\\n')}
` : ''}
END:VCARD
`.trim()
    ...
    async sendButLok(jid, buffer, content, footer, button1, row1, quoted, options = {}) {
      return await this.sendMessage(jid, {
