let handler = async (m) => {
    let doaharian = JSON.stringify(global.doaharian)
    let json = JSON.parse(doaharian)
    let data = json.result.data.map((v, i) => `${i + 1}. ${v.title}\n${v.arabic}\n${v.latin}\n*Artinya:*\n_"${v.translation}"_`).join('\n\n')
    contoh = `*「 Doa Harian 」*\n\n`
    anjuran = `\n\nSeperti yang kita ketahui, berdoa merupakan salah satu amalan yang sangat tidak terlepas dari kehidupan kita. Bagaimana tidak, kita sebagai manusia biasa yang tak memiliki daya dan kekuatan apapun tanpa kehendak Allah menjadikan kita harus memanjatkan doa kepada Allah. Perintah berdoa tertuang jelas pada surat Al Mukmin ayat 60 berikut ini:
